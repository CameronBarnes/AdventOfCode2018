...
impl Shift {
    fn minutes_sleeping(&self) -> usize {
        self.ranges.iter().map(|(start, end)| end - start).sum()
    }

    fn most_asleep(&self) -> usize {
...
pub fn process(input: &str) -> String {
    ...
    for (id, shift) in &shifts {
        let minutes = shift.minutes_sleeping();
    ...
    let best_minute = shifts.get(&best_id).unwrap().most_asleep();
